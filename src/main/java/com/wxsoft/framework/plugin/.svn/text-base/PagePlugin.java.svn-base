package com.wxsoft.framework.plugin;
...
import com.wxsoft.framework.util.ReflectHelper;
			...
			if (mappedStatement.getId().matches(pageSqlId)) { // 拦截需要分页的SQL
				...
				if (parameterObject == null) {
					...
					if (parameterObject instanceof AjaxPage) { // 参数就是Page实体
						...
						Field pageField = ReflectHelper.getFieldByFieldName(
								parameterObject, "ajaxPage");
						if (pageField != null) {
							page = (AjaxPage) ReflectHelper
									.getValueByFieldName(parameterObject,
											"ajaxPage");
							if (page == null)
								page = new AjaxPage();
							...
							page.setTotalResult(count);
							ReflectHelper.setValueByFieldName(parameterObject,
									"ajaxPage", page); // 通过反射，对实体对象设置分页对象
						} else {
							throw new NoSuchFieldException(parameterObject
									.getClass().getName() + "不存在 page 属性！");
						}
		...
		if (page != null && Tools.notEmpty(dialect)) {
			...
			if ("mysql".equals(dialect)) {
				...
				if (sql.indexOf("from company") > -1) {
					...
					pageSql.append(sql);
					pageSql.append(") as o where rownumber>"
							+ page.getCurrentResult() * page.getShowCount());
				} else {
					pageSql.append("select top "
							+ page.getShowCount()
