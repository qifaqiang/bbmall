...
//				} else if ( imgStr.startsWith("data:image/jpeg;base64,") ) {
//					imgStr = imgStr.replace("data:image/jpeg;base64,","");
//				}
				if (imgStr.contains(";base64") ) {
					imgStr = imgStr.split(";base64,").length > 1 ? imgStr.split(";base64,")[1] : imgStr.split(";base64,")[0];
				}
	 ...
	 *
	 * @param imgStr   base64
	 * @param imgFilePath  图片 绝对路径
	 * @param widthTemp
	 * @param heightTemp
	 * @return
	 */
	public static boolean GenerateImageAndRectangle(String imgStr, String imgFilePath,Integer[] widthTemp, Integer[] heightTemp) {
		if (StringUtils.isNotBlank(imgStr)) {
			BASE64Decoder decoder = new BASE64Decoder();
			try {
				// Base64解码
...
//				}
				...
				return Tools.RectangleImage(imgFilePath,bytes,widthTemp, heightTemp);
			} catch (Exception e) {
				return false;
			}
		} else {
