...
@RequestMapping(value = SystemConfig.FXSHOP_ANNOTATION_NAMESPACE_EDITOR)
public class KindeditorController extends BaseController {
	...
	public void uploadJson(HttpServletResponse response,
		...
		String saveUrl = BaseConfig.FX_DOMAIN_WWW + "/attached/";
		// 定义允许上传的文件扩展名
		...
		// dirFile.mkdirs();
		// }
		Map<String, String> maps = mkdir(saveUrl, dirName);
		File dirFile = new File(maps.get("savePath"));
		savePath = maps.get("savePath");
		saveUrl = maps.get("saveUrl");
	...
	public static Map<String, String> mkdir(String saveUrl, String dirName) {
		String savePath = BaseConfig.FX_PIC_PATH + "/attached/";
		saveUrl += dirName + "/";
		savePath += dirName + "/";
		File saveDirFile = new File(savePath);
		if (!saveDirFile.exists()) {
	...
	public void fileManagerJson(HttpServletResponse response,
		...
		String rootPath = request.getSession().getServletContext()
				.getRealPath("/")
				+ "/attached/" + path1 + "/";
		String rootUrl = request.getContextPath() + "/attached/" + path1 + "/";
		// 图片扩展名
