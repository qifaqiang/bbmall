...
{{for(var i=0;i<it.length;i++){ }}
    {{var product=it[i];}}
        <tr class="">
			...
			<td><span id="ct_{{=product.type}}_{{=product.id}}_{{=product.productId}}">{{=product.inventorycount}}&nbsp;{{=product.unit}}</span></td>
		</tr>
{{}}}
</script>
<!-- END PAGE HEADER-->
<!-- BEGIN PAGE CONTENT-->
<form action="kucunlist.html" method="post" id="kucunlist">
	<div class="row">
			...
			<div class="portlet box blue">
				...
				<div class="portlet-body">
					<div class="input-group"
						style="text-align: left; margin-bottom: 3px;" id="search_div">
						<select style="width: 150px; float: right;margin-right: 10px;margin-top: 3px"
							...
							<c:forEach items="${CompanyList}" var="p" varStatus="vs">
								<option value="${p.companyId}"  comNam="${p.companyName}">${p.companyName }</option>
							</c:forEach>
						</select> <span id="zuye" style="float:right;height:35px;line-height:35px">基地商户名称:&nbsp;</span>
	...
	<a id="rolId"  value="${rolId}"></a>
	<a id="companyId"  value="${companyId}"></a>
</form>
<script>
	$(function(){
		$("#companyId").val("");
		
		if($("#rolId").attr("value") == 40){
			$("#search_div").hide();
			showallprod(${companyId},1);
		}
	...
	function showallprod(companyId,currentPage){
		...
		if("" != companyId){
			...
			$.ajax({
				url : "${SHOPDOMAIN}/system/prodBasic/getallprodbycompanyid.html",
				type : "post",
				dataType : "json",
				sync : false,
				data : {
