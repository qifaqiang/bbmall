			...
			<div class="portlet box blue">
				...
				<div class="portlet-body">
					<div class="col-md-4" style="width:1050px;">
						<div class="controls">
							 <input type="text" id="pickCode" style="width: 150px;float:left"
								name="pickCode" placeholder="提货码" class="form-control"
								value="${pickCode }" class="span4"> <input type="text"
								id="ordersn" style="width: 160px;margin-left:15px;float:left" name="ordersn"
								...
								class="span4">
							<div class="input-prepend input-group" style="margin-left:340px;">
								...
								<div id="tab_1_1" class="tab-pane active">
									<div class="table-responsive" style="margin-top: 10px">
										...
										<c:if test="${ orders.status==22}">
											...
											</a> --> &nbsp; <a href="javascript:void(0)" onclick="toexcelbatch()"
												class="btn btn-sm green"> 导出未发货的订单 <i
												class="fa fa-share"></i>
										...
										<table class="table table-bordered table-hover"
											...
											<thead>
												<tr>
													<th style="text-align: center;width:25%">商品</th>
												...
												<c:choose>
													<c:when test="${not empty orResultlist}">
														<c:forEach items="${orResultlist}" var="ords"
															...
															<tr class="">
																<td
																	style="width:30%;width:270px;text-align: center;vertical-align: middle;"><c:forEach
																		items="${ords.od}" var="products">
																		<div style="">
																			<div style="float: left">
																				<img width="50px" src="${USERIMGSRC }${fn:replace(products.prodUri,'.','_160.')}" />
																			</div>
																			<div style="float: left;">
																...
																<td
																	...
																	<c:if test="${ords.userShipType==1}">
																		...
																		提货码：<br>${ords.pickCode }</c:if></td>
																<td
																	style="width:10%;text-align: center;vertical-align: middle;">￥${empty ords.orderPrice?"0.00":ords.orderPrice}
																	<c:if test="${not empty ords.firstOrderPrice}"><br>首单立减￥${ords.firstOrderPrice }</c:if></td>
																<td
																...
																<td
																	style="width:10%;text-align: center;vertical-align: middle;"><button type="button"
																		class="btn btn-sm blue" onclick="toinfo(${ords.id})">
																		查看详情</button><br> <c:choose>
																		<c:when test="${ ords.status==22}">
																			<!-- <button type="button" class="btn   green" style="margin-top: 10px"
																				...
																				修改快递单号</button>
																			<br> -->
																			<button type="button" class="btn btn-sm green" style="margin-top: 10px"
																				onclick="fahuo(${ords.id})">标记发货</button>
																			<br>
																		</c:when>
																	</c:choose> <c:choose>
																		<c:when test="${ ords.status==11}">
																			<button type="button" class="btn btn-sm green" style="margin-top: 10px"
																				...
																				修改价格</button><button type="button" class="btn btn-sm red" style="margin-top: 10px"
...
<script>
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/updateship.html',
											type : 'post',
											dataType : 'json',
											data : {
												orderid : id,
						...
						main : {
							...
							callback : function() {
								if (MoneyCheck($("#orderPricec").val())
									...
									$.ajax({
												url : '${SHOPDOMAIN}/system/ordersBase/updatePrice.html',
												type : 'post',
												dataType : 'json',
												data : {
													orderid : id,
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/addshipbatch.html',
											type : 'post',
											dataType : 'json',
											data : {
												ttime : $("#ttime").val(),
	...
	function toexcelbatch() {
		window
				.open("${SHOPDOMAIN}/system/ordersBase/toexcelbatch.html?typeid=${param.typeid}&ttime="
						+ $("#ttime").val()+"&pickCode="+$("#pickCode").val())+ "&tradingCode="
						+ $("#tradingCode").val();
	}
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/tosendbatch.html',
											type : 'post',
											dataType : 'json',
											data : {
												ttime : $("#ttime").val()
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/fahuo.html',
											type : 'post',
											dataType : 'json',
											data : {
												orderid : id
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/receivingOrderBatch.html',
											type : 'post',
											dataType : 'json',
											beforeSend : function() {
												showTip('正在接单中请稍后……');
					...
					buttons : {
						main : {
							...
							callback : function() {
								$
										.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/receivingOrder.html',
											type : 'post',
											dataType : 'json',
											data : {
												orderid : orderid
				...
				.dialog({
					...
					buttons : {
						main : {
							...
							callback : function() {
								$.ajax({
											url : '${SHOPDOMAIN}/system/ordersBase/cancelOrder.html',
											type : 'post',
											dataType : 'json',
											data : {
												orderid : orderid
	...
	function chargeCompany(id) {
		...
		<c:forEach items="${ListCompanyName}" var="lc"> temp+="<option value='"+${lc.companyId}+"'>${lc.companyName}</option>";</c:forEach>
		...
		bootbox.dialog({
			...
			buttons : {
				main : {
					...
					callback : function() {
						$.ajax({
								url : '${SHOPDOMAIN}/system/ordersBase/chargeCompany.html',
								type : 'post',
								dataType : 'json',
								data : {
									orderid : id,
									newCompanyId:$("#companyBaseId").find("option:selected").val()
								},
								success : function(data) {
									if (data.Status == "1") {
										window.location.href = window.location.href;
	...
	function gotos(ty) {
		window.location.href = "orderslist.html?typeid=${param.typeid}&ty="
			+ ty + "&ttime=" + $("#ttime").val() + "&pickCode=" + $("#pickCode").val()+"&ordersn="+$("#ordersn").val();
}

	function toexcel() {
		window
		.open("${SHOPDOMAIN}/system/ordersBase/toexcel.html?typeid=${param.typeid}&ttime="
				+ $("#ttime").val() + "&ty=${param.ty}"+"&pickCode="+$("#pickCode").val())+ "&tradingCode="
				+ $("#tradingCode").val();
	}
	...
	function print(orderId) {
		...
		window
		.open("${SHOPDOMAIN}/system/ordersBase/toprint.html?orderid="+orderId,
				"小票打印",
