...
<!-- /.modal -->
<!-- BEGIN PAGE HEADER-->
<h3 class="page-title">入账资金统计(统计根据不可退货的订单，各个基地整月的收入和退款金额)</h3>
<link rel="stylesheet" type="text/css"
					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<th colspan="3">支付宝支付</th>
								<th colspan="3">银联支付</th>
								<th colspan="3">合计</th>
							</tr>
						</thead>
						<tbody id="tbodyss">
...
<script type="text/javascript">
	...
	function fun1() {
		$.ajax({
			url : "${SHOPDOMAIN}/system/statistic/moneyadminMonth.html",
			type : "post",
			dataType : "json",
			async : false,
			data : {
			...
			success : function(data) {
				...
				$.each(data.mapAll, function(index, item) {
					...
					html+="</td><td>实收："+weixinzhifuyingshou.toFixed(2)+"</td><td>退款："+weixinzhifutuikuan.toFixed(2)+"</td><td>入账："+weixinzhifushishou.toFixed(2)+
					"</td><td>实收："+zhifubaozhifuyingshou.toFixed(2)+"</td><td>退款："+zhifubaozhifutuikuan.toFixed(2)+"</td><td>入账："+zhifubaozhifushishou.toFixed(2)+
					"</td><td>实收："+yinlianzhifuyingshou.toFixed(2)+"</td><td>退款："+yinlianzhifutuikuan.toFixed(2)+"</td><td>入账："+yinlianzhifushishou.toFixed(2)+
					"</td><td>实收："+hezhifuyingshou.toFixed(2)+"</td><td>退款："+hezhifutuikuan.toFixed(2)+"</td><td>入账："+hezhifushishou.toFixed(2)+"</td></tr>";
				})
				var tip="<div class='divFloat'>微信支付实收:"+allweixinzhifuyingshou.toFixed(2)+" </div><div class='divFloat'>退款:"+allweixinzhifutuikuan.toFixed(2)+" </div><div class='divFloat'>入账:"+allweixinzhifushishou.toFixed(2)+
				"</div><div class='divFloat'>支付宝支付实收:"+allzhifubaozhifuyingshou.toFixed(2)+" </div><div class='divFloat'>退款:"+allzhifubaozhifutuikuan.toFixed(2)+" </div><div class='divFloat'>入账:"+allzhifubaozhifushishou.toFixed(2)+
				"</div><div class='divFloat'>银联支付实收:"+allyinlianzhifuyingshou.toFixed(2)+" </div><div class='divFloat'>退款:"+allyinlianzhifutuikuan.toFixed(2)+" </div><div class='divFloat'>入账:"+allyinlianzhifushishou.toFixed(2)+
				"</div><div class='divFloat'>合计实收:"+allzhifushishou.toFixed(2)+" </div><div class='divFloat'>退款:"+allzhifutuikuan.toFixed(2)+" </div><div class='divFloat'>入账:"+allzhifushishou.toFixed(2)+"</div>";
	...
	function toexcel() {
		window
				.open("${SHOPDOMAIN}/system/statistic/moneyadminexcel.html?terms="
						+ $(".form_datetime").val());
	}
</script>
<script
