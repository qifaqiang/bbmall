...
<div class="row">
	<div class="col-md-12">
		...
		<ul class="page-breadcrumb breadcrumb">
			...
			<li><a href="javascript:void(0)"> 退货订单详情</a></li>
		</ul>
	...
	<div class="col-md-12">
			...
			<div class="portlet-body form">
				<div class="form-wizard">
					<div class="navbar-inner">
						<ul class="row-fluid nav nav-pills steps">
							<li class="span3 active" style="width: 16.5%"><a
								class="step" style="background-color: #fff;" href="#tab1"> <span
									class="number">1</span> <span class="desc"
							...
							<li
								class="span3 <c:if test="${orders.status==40||orders.status==50||orders.status==60 }">active</c:if>"
								style="width: 16.5%"><a class="step"
							...
							<li
								class="span3 <c:if test="${orders.status==50||orders.status==60}">active</c:if>"
								style="width: 16.5%"><a class="step"
								style="background-color: #fff;" href="javascript:void(0)"> <span
									class="number">5</span> <span class="desc"
									style="font-size:10pt"><i class="icon-ok"></i> 申请退款 <fmt:formatDate
											value="${orders.rg.addtime}" type="both"
											pattern="yyyy/MM/dd HH:mm:ss" /></span>
							</a></li>
							<li class="span3 <c:if test="${orders.status==60}">active</c:if>"
								style="width: 16.5%"><a class="step"
								style="background-color: #fff;" href="javascript:void(0)"> <span
									class="number">6</span> <span class="desc"
									style="font-size:10pt"><i class="icon-ok"></i> 系统审核 <fmt:formatDate
											value="${orders.rg.recordtime}" type="both"
											pattern="yyyy/MM/dd HH:mm:ss" /></span>
							</a></li>
						</ul>
					</div>
					<div class="progress progress-success progress-striped" id="bar">
						<div class="progress-bar"
							style="width: <c:choose>
									<c:when test="${ orders.status==11}">16.5%</c:when>
									<c:when test="${ orders.status==20}">33%</c:when>
									<c:when test="${ orders.status==30}">49.5%</c:when>
									<c:when test="${ orders.status==40}">66%</c:when>
									<c:when test="${ orders.status==50}">82.5%</c:when>
									<c:when test="${ orders.status==60}">99%</c:when>
									<c:when test="${ orders.status==0}">16.5%</c:when>
								</c:choose>;"></div>
					</div>
				</div>
				<!-- BEGIN FORM-->
				<form class="form-horizontal" id="form_config" method="post"
					...
					<div class="form-body"
						...
						<c:if test="${not empty ords.firstOrderPrice}">
						<div class="form-group">
							<label class="control-label col-md-3"
			...
			<div class="portlet-body form">
				...
				<form class="form-horizontal" id="form_config" method="post"
					...
					<div class="form-body">
						<div class="form-group">
							<label class="control-label col-md-3"
								style="text-align: left;width: 100%">用户退货理由
								：${orders.rg.remark }</label>
						</div>
						<div class="form-group">
							<label class="control-label col-md-3"
								style="text-align: left;width: 100%">系统审核状态 ：<c:if
									test="${orders.rg.status==0}">审核中</c:if> <c:if
									test="${orders.rg.status==1}">审核通过</c:if> <c:if
									test="${orders.rg.status==2}">审核不通过</c:if>
							</label>
						</div>
						<div class="form-group">
							<label class="control-label col-md-3"
								style="text-align: left;width: 100%">系统审核批注
								：${orders.rg.sysRemark } </label>
						</div>
		...
		<div class="portlet box blue">
			...
			<div class="portlet-body ">
				...
				<table class="table table-bordered table-hover">
					...
					<tbody>
						<c:forEach items="${orders.od }" var="ods">
							<tr class="">
								<td><img alt="" src="${USERIMGSRC }/uploads/${ods.prodUri}"
									width="50px">${ods.prodName }</td>
				...
				<div style="float: right">>商品+优惠+运费小计:${orders.orderAccount }</div>
				<br> <br>
				<!-- END FORM-->
			</div>
		</div>
